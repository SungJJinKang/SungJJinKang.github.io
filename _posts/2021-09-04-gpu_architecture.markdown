...
이렇게 KMD로 매핑받은 메모리 공간을 통해서 텍스쳐나 버텍스 데이터를 전송하는데, CPU가 DRAM에 있는 텍스쳐, 버텍스 데이터를 전송하려면 우선 KMD에서 매핑 받은 메모리 공간으로 DRAM ( 기존의 텍스쳐, 버텍스 데이터가 있는 위치) -> DRAM ( KMD가 매핑해준 메모리 공간)으로 데이터 복사를 한 후 API를 통해 GPU가 해당 DRAM의 텍스쳐, 버텍스 데이터가 복사된 위치에 접근하여 데이터를 복사해간다. ( 후술할거지만 GPU에서 DMA를 통해 DRAM에 매핑된 텍스쳐, 버텍스 데이터들을 복사하는 것이다. )                     
