...
반면 IOS Metal은 그렇지 않다. IOS Metal의 경우 OS단에서 PSO를 캐싱해둔다. 디스크에 말이다. 그래서 IOS에서 게임을 플레이하면서 PSO Hitch가 한번 발생한 부분은 앱을 껏다 킨 후 동일한 구간을 플레이 했을 때 PSO Hitch가 발생하지 않는다. ( reference : http://geekfaner.com/shineengine/WWDC2020_BuildGPUbinarieswithMetal.html )                         
