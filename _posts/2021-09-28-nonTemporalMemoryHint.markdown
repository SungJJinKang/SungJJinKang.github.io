...
그래서 Write Combined 버퍼 ( 사이즈가 캐시라인 사이즈와 같다 )를 완전히 채워서 데이터를 전송하는 경우 한번의 버스 트랜젝션 ( 버스트 모드 )으로 처리가 가능하지만, 만약 Write Combined 버퍼를 완전히 채우지 않은 상태로 버스 트랜젝션을 수행하는 경우 8번 ( 64바이트/8바이트, 64비트 CPU의 경우 한번의 버스 트랜젝션에 64비트 즉 8바이트를 전송할 수 있다. ) 혹은 4번 ( 32/8 )의 버스 트랜젝션 ( 일반 모드로 한번의 버스 트랜젝션 당 8바이트씩만 처리 가능 ) 을 통해서 처리해야한다.       
