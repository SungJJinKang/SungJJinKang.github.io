...
일반적인 경우에서는 Occlusion Culling을 끄는 것이 낫다. 오히려 Occlusion Culling 과정에 Depth Buffer를 그리는데 더 시간이 많이 소모된다.       
다만 최악의 경우 ( 그려지는 오브젝트가 많은 경우 ) 에는 프레임 향상이 많이된다.                    
즉 **평균 프레임은 조금 떨어지지만 최저 프레임의 향상을 가져오는 것**이다.           

필자가 생각하기에 어쨌든 게임이라는 것은 최고 프레임보다는 평균적으로 균일하게 프레임을 보장해주는 것이 중요하다고 생각한다.                       
**최악의 상황에서 프레임이 뚝 떨어지는 것보다는 평균 프레임은 조금 떨어지더라도 프레임이 갑자기 뚝 떨어지는 것을 막는 것이 중요**하다는 것이다.      
그렇기에 이 관점에서 본다면 **결과는 나름 성공적**이라고 생각한다.                    
