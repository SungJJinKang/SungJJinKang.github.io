...
코드를 직접 보지는 못했지만 언리얼 엔진의 IsValid와 같이 해당 UObject가 valid한지를 판단하기 위해서는 이 글에서와 같이 UObject들의 주소를 해쉬테이블로 관리해서 주소가 전달되면 해당 주소를 키 값으로 탐색을 수행해 valid한지를 판단할 것 같다.         
