...
- **Unified Memory Architure** : CPU와 GPU가 물리 메모리를 공유. CPU와 GPU의 MMU ( 가상 주소를 물리적 주소로 변환해주는 하드웨어 장치. [참고](https://sungjjinkang.github.io/virtualmemoryaddress) ) 가 같은 물리적 주소 공간을 사용. CPU -> GPU로의 메모리 복사가 필요없음. 그냥 GPU에서 바로 주소 참조해서 접근하면 된다.          
